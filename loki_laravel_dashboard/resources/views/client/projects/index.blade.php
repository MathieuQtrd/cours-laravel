                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <table class="w-full border-collapse text-left border">
                        <tr class="border">
                            ...
                            <th class="p-3 border">Valider</th>
                        </tr>
                        @foreach($projects AS $project)
                            <tr class="border">
                                ...
                                <td class="p-3 border"><a href="{{ route('client.projects.show', $project->id) }}" class="bg-teal-500 text-white px-3 py-1 rounded inline-block">Détails</a></td>
                                <td class="p-3 border">
                                    @if($project->status == 'terminé')
                                    <form action="" method="POST">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" onclick="return(confirm('Etes vous sur ?'))" class="bg-green-500 text-white px-3 py-1 rounded">Valider</button>
                                    </form>
                                    @endif
                                </td>
                            </tr>
